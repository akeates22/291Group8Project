...
namespace movieRentalApp
{
    public partial class Rate_Movie : Form
    {
        ...
        public void updateMovieAvg(string movie, int newRating)
        {
            SqlConnection myConnection = new SqlConnection(connectionString);
            try
            {
                ...
                string getRatingCount = "select count(*) from orders O, movies M where M.movieId = O.movieID and " +
                                        "M.movieName = '" + movie + "' and O.rating is not null;";
                SqlCommand cmd = new SqlCommand(getRatingCount, myConnection);
                int totalRatings = Convert.ToInt32(cmd.ExecuteScalar());

                cmd.CommandText = "select ISNULL(rating,0) from movies where movieName = '" + movie + "';";
        ...
        public void updateActorAvg(string movie, int rating)
        {
            SqlConnection myConnection = new SqlConnection(connectionString);
            try
            {
                myConnection.Open();
                // get list of actors in a given movie
                ...
                string actorData = "select distinct A.actorID, ISNULL(A.rating,0), temp.count from actors A, movies_in MI, " +
                ...
                while (reader.Read())
                {
                    ...
                    decimal newAvgExact = ((rating + ((ratingCount - 1) * currAvg)) / ratingCount);
                    int newAvg = (int)Math.Round(newAvgExact, 0);
